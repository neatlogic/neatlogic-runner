...
public class SvnAgentUtils {
    ...
    public static void createRepo(JSONObject jsonObj) {
        String createRepoUserId = UserContext.get().getUserId();
        ...
        try {
            result = post(url, jsonObject, getAuthHeader(username, password));
            JSONObject createRepoReturnJsonObj = JSONObject.parseObject(result);
            ...
            throw new ApiRuntimeException("仓库授权失败，错误信息：" + result);
        }
    ...
    public static boolean getDelegation(JSONObject jsonObj) throws UnsupportedEncodingException {
        ...
        String username = jsonObj.getString("agentUsername");
        String password = jsonObj.getString("agentPassword");
        String repo = jsonObj.getString("repo") == null ? StringUtils.EMPTY : jsonObj.getString("repo");
