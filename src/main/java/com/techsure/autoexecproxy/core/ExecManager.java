...
public class ExecManager {
    ...
    public static void exec(CommandVo commandVo) throws Exception {
        ...
        //set command
        commandVo.setCommandList(Collections.singletonList(String.format("%s python3 %s/bin/autoexec --jobid \"%s\" --execuser \"%s\" -p %s",COMMAND_AUTOEXEC,Config.AUTOEXEC_HOME(),commandVo.getJobId(),commandVo.getExecUser(),filePath)));
